# affine 路 —— `--convert-affine-for-to-gpu`（规整循环直接映射成网格）

> CUDA core(标量)路下的**两条子路之一**。总览(两条子路对比)见 [`../README.md`](../README.md);
> GPU 顶层见 [`../../README.md`](../../README.md);张量核那条见 [`../../tensor_core/`](../../tensor_core/README.md)。
> **这条把规整的 `affine.for` 直接映射成网格:m→block、n→thread,落到 CUDA core 的标量 `FMUL`/`FADD`——碰不到 tensor core。**

前面 `../../../matmul_hello_world/`、`../../../matmul/` 都是把 `linalg.matmul` 降到 **CPU** 的 `.ll`。这个例子换目标:
**同一个手写源 `matmul.mlir`(从 hello_world 直接 cp),一路降到 GPU**——经 `gpu`→`nvvm` 方言,
...
> **和隔壁 [`../parallel/`](../parallel/README.md)(官方 `--convert-parallel-loops-to-gpu`)的区别就在网格:**
> 这条 affine 路是「**m 个 block,每 block n 个线程**」(4 block × 16 线程);parallel 那条是
> 「**m×n 个 block,每 block 1 个线程**」(4×16 block × 1 线程)。两条都对、都落到标量 `FMUL/FADD`,
> 只是把并行维摊进网格的方式不同——细看 `../README.md` 的对比表。
...
| `../../extract_ptx.py` | 手写工具(两路共用) | 从 `gpu.binary` 字符串里抠出干净 PTX(解 `\XX` 转义) |
...
`make artifacts` 一把出全部;`make clean` 只留 `matmul.mlir`(PTX 提取器在 `../../extract_ptx.py`)。
...
> 注:这里 K=8 在 PTX 里是**展开**的(8 组 `mul.rn`/`add.rn`,无回边)。隔壁 parallel 路同样的 NVVM(都是
> 1 条 `llvm.fmul` + rolled 循环)出 PTX 时**没被展开**——差异在 LLVM **NVPTX 后端的循环展开启发式**,
> 不在 MLIR 这层(实测两边 NVVM 一致)。
...
make clean       # 只留 matmul.mlir（PTX 提取器在 ../../extract_ptx.py）
```

## 注意 / 下一步

- 这是**最朴素**的映射:一个线程算一个 `C[m,n]`,k 顺序累加,**没用 shared memory、没分块、没 tensor core**。
  真正高性能的 GPU matmul 要 tile 到 shared memory、用 `nvgpu`/`mma`(tensor core)——见 `../../tensor_core/`。
